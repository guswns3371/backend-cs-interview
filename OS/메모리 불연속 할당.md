...
- `각 segment 마다 valid bit를 둔다`
    - 각 segment의 주소 변환 정보가 유효한지, 즉 해당 세그먼트가 현재 물리적 메모리에 적재되어 있는 지를 나타낸다. (paging도 마찬가지)
...
| Paging : page table | Segmentation : segment table |
| --- | --- |
| page의 크기가 균일하기 때문에 offset의 크기가 page 크기에 의해 결정된다. | segment의 길이는 offset이 표현가능한 범위내에 제한 되어야 한다. |
...
프로그램의 주소 공간을 segment로 나누되, segment마다 page로 나누는 기법
...
2. segmentTable[segment number] = “`segment length`- `page table base`” 정보 참조
3. page table base로 segment 내의 page table에 접근
4. offset(d) 는 segment 내의 page number(p)와 해당 page 에서의 변위(d’)로 이루어져있다. p와 d’를 통해 실제 물리 메모리에 접근
