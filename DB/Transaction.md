...
DB에서 `동시성 제어` 란 데이터 일관성을 유지하기 위해 동시에 수행되는 트랜잭션의 실행 순서를 제어하는 기법이다.

> 동시성 제어 방식
> 
1. `Locking` : 하나의 트랜잭션이 연산을 수행할 때 데이터에 Lock을 설정하면, 다른 트랜잭션은 Lock이 걸린 데이터가 Unlock될 때까지 접근/수정/삭제하지 못하도록 하는 제어 기법
    - `S-Lock(Shared Lock)` : 데이터에 읽기 연산을 수행할 때 설정하는 Lock
        - 여러 S-Lock은 동시에 적용될 수 있다. (데이터 일관성을 해치지 않기 때문)
    - `X-Lock(Exclusive Lock)` : 데이터에 쓰기 연산을 수행할 때 설정하는 Lock
        - 하나의 트랜잭션이 데이터에 X-Lock를 걸면 트랜잭션이 종료될 때까지 X-Lock이 유지된다.
        - X-Lock이 걸린 데이터에 다른 트랜잭션이 S-Lock, X-Lock을 걸 수 없다
2. `Timestamp` : Timestamp(시스템에서 생성되는 고유 번호)를 트랜잭션에 부여하여 트랜잭션간의 접근 순서를 미리 정하는 제어 기법
    - 먼저 들어온(Timestamp가 더 빠른) 트랜잭션이 먼저 수행
3. `Validation` : 메모리 상의 복사본에서 트랜잭션을 수행시키고, 완료될 때 적합성 검증 후 실제 DB에 반영하는 제어 기법

> 동시성 제어를 하지 않을 경우 발생하는 문제
> 
- `Lost Update`
    
    둘 이상의 트랜잭션이 하나의 데이터에 Update 연산을 수행할 때, 마지막에 완료된 트랜잭션의 결과로 덮어씌워져 갱신이 무효화 되는 문제
    
    ```java
    1. 트랜잭션1이 데이터A를 갱신
    2. 트랜잭션2가 데이터A를 갱신
    
    결국 트랜잭션1의 결과는 무효화 되고 데이터A에는 트랜잭션2의 결과만 반영된다.
    ```
...
- `Inconsistency`
    ...
    ```java
    1. 트랜잭션1이 데이터A를 읽음
    2. 트랜잭션2가 데이터A를 갱신 후 commit
    3. 트랜잭션3가 변경된 데이터A를 읽음
    
    트랜잭션1과 3가 읽은 데이터A의 값이 서로 다르다
    ```
    
- `Cascading Rollback` (Dirty Read가 포함된 문제)
    
    트랜잭션1이 데이터를 갱신하는 중간에 트랜잭션2가 데이터를 읽고 commit한 경우, 트랜잭션1이 rollback한다면 트랜잭션2는 rollback하지 못하는 문제
    
    ```java
    1. 트랜잭션1이 데이터A를 갱신하고 있음
    2. 트랜잭션2가 갱신되고 있는 데이터A를 읽고 작업을 수행한 뒤 commit
    3. 트랜잭션1이 비정상 종료되어 rollback
    
    트랜잭션2는 결과적으로 잘못된 데이터로 작업을 수행하여 이미 commit되었다. 즉, rollback할 수 없다.
    ```
...
- `Dirty Read`
    ...
    ```java
    1. 트랜잭션1이 데이터A를 갱신하고 있음
    2. 트랜잭션2가 갱신중인 데이터A를 읽음
    3. 트랜잭션1이 rollback
    
    트랜잭션2는 rollback되어 갱신이 무효화된 데이터A를 가지게 된다.
    ```
    
- `Non-Repeatable Read`
    
    한 트랜잭션에서 같은 쿼리를 두 번 수행할 때, 동일한 결과가 나오지 않는 문제(이전 결과와 현재 결과가 다르다)
    
    ```java
    1. 트랜잭션1이 데이터A를 읽음 -> "데이터A : 1"
    2. 트랜잭션2이 데이터A를 갱신함 -> "데이터A : 2"
    3. 트랜잭션1이 데이터A를 읽음 -> "데이터A : 2" // 이전 결과와 다르다
    ```
    
- `Phantom Read`
    
    일정 범위의 레코드를 두 번 이상 읽었을 때, 이전에 없던 레코드가 나타나는 현상
    
    ```java
    1. 트랜잭션1이 테이블T를 읽음 -> "데이터A : 1"
